  ...
  function onTick(t,price){
    ...
    for(const [lbl,per] of Object.entries(TF)){
      ...
      if(slot>b.t){
        ...
        if(lbl==='15s'){
          if (window.ENABLE_DECISION_BY_HTF) {
            try{ if (typeof HTF_onClose15s==='function') HTF_onClose15s(closedBar); }catch(e){ console.error('[HTF] 15s-update error:', e&&e.message? e.message:e); }
          }
        }
        if(lbl==='1m'){
...
let makeBet = (priceTrend, tradeStatus, globalTrend) => {
    ...
    if (window.ENABLE_DECISION_BY_HTF && window.__HTF__) {
        ...
        if (htfDecisionSide === 'SKIP') {
            ...
            HTF_markSkip('badSignals');
            return;
        }
        if (!Number.isFinite(htfDecisionTs)) {
            HTF_markSkip('nanGuard');
            return;
        }
        if (window.__HTF__.lastOrderTs === htfDecisionTs) {
            HTF_markSkip('duplicateTs');
            return;
        }
        tradeDirection = htfDecisionSide === 'UP' ? 'buy' : 'sell';
        tradeDirectionDiv.style.background = '#c98a02';
        usingHTFDecision = true;
    }
    ...
    if (currentProfit > limitWin){
        ...
        if (window.__HTF__) {
            window.__HTF__.tradesCount = (typeof window.__HTF__.tradesCount === 'number' ? window.__HTF__.tradesCount : 0) + 1;
        }
...
window.HTF_CONFIG.verbose = window.HTF_CONFIG.verbose === true;
window.HTF_CONFIG.debugLogs = (window.HTF_CONFIG.verbose === true) || window.HTF_CONFIG.debugLogs === true;
window.HTF_CONFIG.statsLogPeriodMin = window.HTF_CONFIG.statsLogPeriodMin || 5;

function HTF_isVerbose(){
  return !!(window.HTF_CONFIG && (window.HTF_CONFIG.verbose || window.HTF_CONFIG.debugLogs));
}

window.__HTF__ = window.__HTF__ || {
  ready: false,
...
window.__HTF__.skipStats = window.__HTF__.skipStats || {
  notReady: 0,
  badPayout: 0,
  badSignals: 0,
  nanGuard: 0,
...
window.__HTF__.lastStatsLogTs = window.__HTF__.lastStatsLogTs || null;
window.__HTF__.tradesCount = (typeof window.__HTF__.tradesCount === 'number') ? window.__HTF__.tradesCount : 0;

function HTF_markSkip(reason) {
  if (!window.__HTF__ || !window.__HTF__.skipStats) return;
  var s = window.__HTF__.skipStats;
  if (reason === 'notReady') s.notReady++;
  else if (reason === 'badPayout') s.badPayout++;
  else if (reason === 'badSignals') s.badSignals++;
  else if (reason === 'nanGuard') s.nanGuard++;
  else if (reason === 'duplicateTs') s.duplicateTs++;

  try {
    if (HTF_isVerbose()) console.log('[HTF reason] skip=' + reason);
  } catch(e) {}
}
...
function HTF_maybeLogStats(nowTs) {
  try {
    ...
    var periodMin = cfg.statsLogPeriodMin || 5;
    var last = window.__HTF__.lastStatsLogTs;
    if (last && (nowTs - last) < periodMin*60*1000) return;

    window.__HTF__.lastStatsLogTs = nowTs;

    var s = window.__HTF__.skipStats;
    var tradesCount = (typeof window.__HTF__.tradesCount === 'number') ? window.__HTF__.tradesCount : 0;

    console.log('[HTF stats] period='+periodMin+'m trades='+tradesCount+
      ', skip={signals:'+s.badSignals+', payout:'+s.badPayout+', notReady:'+s.notReady+', duplicate:'+s.duplicateTs+', nan:'+s.nanGuard+'}');
  } catch(e) {
  }
}
...
function HTF_calcDecision(){
  ...
  var trendUp = e10 > e20;
  var trendDown = e10 < e20;
  var phaseUp = rsi >= 50;
  var phaseDown = rsi <= 50;
  var momUp = sm10 > 0;
  var momDown = sm10 < 0;
  flags.trend = trendUp ? 1 : (trendDown ? -1 : 0);
...
function HTF_onClose15s(bar){
  try{
    ...
    if(prev !== decision){
      var color = 'color:#9aa4b2;font-weight:600;';
      ...
      if (HTF_isVerbose()) console.log('%c[HTF decision] NEW → '+decision+simpleTag+' (EMA/RSI/SM = '+flagsStr+')', color);
    }
    if(typeof HTF_renderSignalsLine==='function') HTF_renderSignalsLine();
    return decision;
  }catch(err){ console.error('[HTF] decision error:', err&&err.message? err.message:err); return 'SKIP'; }
}
/* ===== HTF module end ===== */
...
(function(){
  ...
  window.__HTF__.skipStats = window.__HTF__.skipStats || {
    notReady:0,badPayout:0,badSignals:0,nanGuard:0,duplicateTs:0
  };

  var pass = true;
  try {
    if (typeof HTF_markSkip!=='function') {
      console.error('[HTF QA] ❌ no HTF_markSkip'); pass=false;
    }
    if (typeof HTF_maybeLogStats!=='function') {
      console.error('[HTF QA] ❌ no HTF_maybeLogStats'); pass=false;
    }

    var s = window.__HTF__.skipStats;
    var beforeSignals = s.badSignals;
    HTF_markSkip('badSignals');
    if (s.badSignals !== beforeSignals+1) {
      console.error('[HTF QA] ❌ badSignals counter not incremented'); pass=false;
    }

    var beforePayout = s.badPayout;
    HTF_markSkip('badPayout');
    if (s.badPayout !== beforePayout+1) {
      console.error('[HTF QA] ❌ badPayout counter not incremented'); pass=false;
    }

    HTF_maybeLogStats(Date.now());

    if (HAS_DOM && typeof HTF_renderSignalsLine==='function') {
      try { HTF_renderSignalsLine(); }
      catch(e){ console.error('[HTF QA] ❌ render', e && e.message); pass=false; }
    }
  } catch(err) {

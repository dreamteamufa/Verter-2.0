...
let BLOCK_TRADING = false;
const rootWindow = typeof window !== 'undefined' ? window : globalThis;
rootWindow.__BOOT_STATUS__ = { errors: 0, warnings: 0 };
rootWindow.BLOCK_TRADING = BLOCK_TRADING;
let selfTestStatus = 'PASS';
...
function guardElement(element, label) {
    if (!element) {
        rootWindow.__BOOT_STATUS__.errors += 1;
        BLOCK_TRADING = true;
        console.error(`%c[HTF ${APP_VERSION}] Missing DOM element: ${label}`, 'color:#e11d48;font-weight:700;');
    } else {
        QA_COUNTS.domGuards += 1;
    }
    return element;
}

function emaOne(values, period) {
    if (!Array.isArray(values) || values.length < period) {
        return undefined;
    }
    const smoothing = 2 / (period + 1);
    let ema = values.slice(0, period).reduce((acc, num) => acc + num, 0) / period;
...
function standardDeviation(values, mean) {
    ...
    const avg = typeof mean === 'number' ? mean : simpleMovingAverage(values);
...
function averageTrueRange(bars) {
    ...
    for (let i = 0; i < bars.length; i++) {
        const bar = bars[i];
        const highLow = bar.H - bar.L;
        if (i === 0) {
            sum += highLow;
        } else {
            const prevClose = bars[i - 1].C;
            const highClose = Math.abs(bar.H - prevClose);
            const lowClose = Math.abs(bar.L - prevClose);
...
function linearRegression(values) {
    ...
    const slope = ((length * xySum) - (xSum * ySum)) / denominator;
    const intercept = (ySum - (slope * xSum)) / length;
    return (slope * length) + intercept;
}

function squeezeMomentumOne(bars) {
    const length = HTF.need.SM10;
    if (!Array.isArray(bars) || bars.length < length) {
        return undefined;
    }
    const recentBars = bars.slice(-length);
    const closes = recentBars.map(bar => bar.C);
    const highs = recentBars.map(bar => bar.H);
    const lows = recentBars.map(bar => bar.L);
    const basis = simpleMovingAverage(closes);
    const deviation = standardDeviation(closes, basis) * 2;
    const upperBB = basis + deviation;
    const lowerBB = basis - deviation;
    const atr = averageTrueRange(recentBars) * 1.5;
    const upperKC = basis + atr;
    const lowerKC = basis - atr;
    const momentumSource = bars.slice(-10).map(bar => bar.C);
    const highestHigh = Math.max(...bars.slice(-10).map(bar => bar.H));
    const lowestLow = Math.min(...bars.slice(-10).map(bar => bar.L));
    const averageHL = (highestHigh + lowestLow) / 2;
    const averageClose = simpleMovingAverage(momentumSource);
    const adjusted = momentumSource.map(value => value - ((averageHL + averageClose) / 2));
    const momentum = linearRegression(adjusted);
    ...
    return {
        ...
        squeezeOn,
        squeezeOff,
        squeezeNo: !squeezeOn && !squeezeOff
    };
}

function updateWarmupCounts() {
    HTF.warmup.EMA10 = Math.min(HTF.need.EMA10, HTF_INTERNAL.closes.length);
    HTF.warmup.EMA20 = Math.min(HTF.need.EMA20, HTF_INTERNAL.closes.length);
    HTF.warmup.RSI7 = Math.min(HTF.need.RSI7, HTF_INTERNAL.closes.length);
    HTF.warmup.SM10 = Math.min(HTF.need.SM10, HTF_INTERNAL.bars.length);
    HTF.ready = Object.entries(HTF.need).every(([key, min]) => HTF.warmup[key] >= min);
}

function getProgressLabel() {
    const progress = `EMA10 ${HTF.warmup.EMA10}/${HTF.need.EMA10} • EMA20 ${HTF.warmup.EMA20}/${HTF.need.EMA20} • RSI7 ${HTF.warmup.RSI7}/${HTF.need.RSI7} • SM10 ${HTF.warmup.SM10}/${HTF.need.SM10}`;
    return progress + (HTF.ready ? ' [• READY]' : '');
}
...
function handleMinuteClose(bar) {
    HTF_INTERNAL.bars.push(bar);
    HTF_INTERNAL.closes.push(bar.C);
    HTF_INTERNAL.highs.push(bar.H);
    HTF_INTERNAL.lows.push(bar.L);
    if (HTF_INTERNAL.bars.length > 600) {
        HTF_INTERNAL.bars.shift();
    }
    if (HTF_INTERNAL.closes.length > 600) {
        HTF_INTERNAL.closes.shift();
        HTF_INTERNAL.highs.shift();
        HTF_INTERNAL.lows.shift();
    }
    updateWarmupCounts();
    const ema10Value = emaOne(HTF_INTERNAL.closes, HTF.need.EMA10);
    const ema20Value = emaOne(HTF_INTERNAL.closes, HTF.need.EMA20);
    const rsi7Value = rsiOne(HTF_INTERNAL.closes, HTF.need.RSI7);
    const sqz10Value = squeezeMomentumOne(HTF_INTERNAL.bars);
    if (ema10Value !== undefined) {
        HTF.ema10.push(ema10Value);
        if (HTF.ema10.length > 600) {
            HTF.ema10.shift();
        }
    }
    if (ema20Value !== undefined) {
        HTF.ema20.push(ema20Value);
        if (HTF.ema20.length > 600) {
            HTF.ema20.shift();
        }
    }
    if (Number.isFinite(rsi7Value)) {
        HTF.rsi7.push(rsi7Value);
        if (HTF.rsi7.length > 600) {
            HTF.rsi7.shift();
        }
    }
    if (sqz10Value !== undefined) {
        HTF.sqz10.push(sqz10Value);
        if (HTF.sqz10.length > 600) {
            HTF.sqz10.shift();
        }
    }
    updateSignalsProgress();
    const ema10Str = formatValue(HTF.ema10[HTF.ema10.length - 1]);
    const ema20Str = formatValue(HTF.ema20[HTF.ema20.length - 1]);
    const rsi7Str = formatValue(HTF.rsi7[HTF.rsi7.length - 1]);
    const lastSqz = HTF.sqz10[HTF.sqz10.length - 1];
    const sm10Str = lastSqz ? formatValue(lastSqz.value) : formatValue(undefined);
    if (HTF.lastLoggedTs !== bar.t) {
        HTF.lastLoggedTs = bar.t;
        console.log(`%c[HTF ${APP_VERSION}] EMA10=${ema10Str} EMA20=${ema20Str} RSI7=${rsi7Str} SM10=${sm10Str} | progress ${getProgressLabel()}`, 'color:#1e6fff;font-weight:600;');
    }
}

function updateSignalsProgress() {
    if (typeof signalProgressDiv !== 'undefined' && signalProgressDiv) {
        signalProgressDiv.textContent = `Signals: ${getProgressLabel()}`;
    }
}

rootWindow.onerror = function onWindowError(message, source, lineno, colno, error) {
    console.error(`%c[HTF ${APP_VERSION}] Runtime error: ${message}`, 'color:#e11d48;font-weight:700;', source, lineno, colno, error);
    rootWindow.__BOOT_STATUS__.errors += 1;
    BLOCK_TRADING = true;
    rootWindow.BLOCK_TRADING = true;
    if (typeof tradingAllowed !== 'undefined') {
        tradingAllowed = false;
    }
    return false;
};

rootWindow.addEventListener('unhandledrejection', event => {
    console.error(`%c[HTF ${APP_VERSION}] Unhandled rejection`, 'color:#e11d48;font-weight:700;', event.reason);
    rootWindow.__BOOT_STATUS__.errors += 1;
    BLOCK_TRADING = true;
    rootWindow.BLOCK_TRADING = true;
    if (typeof tradingAllowed !== 'undefined') {
...
(function selfTest(){
    ...
    if(emaOne(seq,10)===undefined||!Number.isFinite(rsiOne(seq,7))){
      console.error("%c[HTF] self-test failed","color:#e11d48;font-weight:700;");
      rootWindow.__BOOT_STATUS__.errors++;
      selfTestStatus = 'FAIL';
    }
})();
...
const percentProfitDiv = guardElement(document.getElementsByClassName("value__val-start")[0], 'value__val-start[0]');
if (!percentProfitDiv) {
    return;
}
if (mode === 'REAL'){
    balanceDiv = guardElement(document.getElementsByClassName("js-hd js-balance-real-USD")[0], 'js-hd js-balance-real-USD[0]');
} else {
    balanceDiv = guardElement(document.getElementsByClassName("js-hd js-balance-demo")[0], 'js-hd js-balance-demo[0]');
}
if (!balanceDiv) {
    return;
}
const symbolDiv = guardElement(document.getElementsByClassName("current-symbol")[0], 'current-symbol[0]');
if (!symbolDiv) {
    return;
}
let symbolName = symbolDiv.textContent.replace("/", " ");

const betTimeDiv = guardElement(document.getElementsByClassName("value__val")[0], 'value__val[0]');
if (!betTimeDiv) {
    return;
}
...
if (!targetElem || targetElem.length === 0) {
    guardElement(null, 'tooltip-text');
    return;
}
...
if (!targetElement2) {
    guardElement(null, 'tooltip-text target');
    return;
}
const buyButton = guardElement(document.getElementsByClassName("btn btn-call")[0], 'btn btn-call[0]');
if (!buyButton) {
    return;
}
...
const betInputContainer = guardElement(document.getElementsByClassName("call-put-block__in")[0], 'call-put-block__in[0]');
if (!betInputContainer) {
    return;
}
let betInput = guardElement(betInputContainer.querySelector("input"), 'bet-input');
if (!betInput) {
    return;
}
let betDivContent = betInput.value;
...
let setValue = v => {
    ...
    const input = betInputContainer.querySelector("input");
    if (!input) {
        guardElement(null, 'bet-input');
        return;
    }
    input.value = v;
...
function addUI() {
    ...
    newDiv.innerHTML += '<div id="signal-progress">Signals: '+getProgressLabel()+'</div><br>';
    ...
    const body = guardElement(document.body, 'document.body');
    if (!body) {
        return;
    }
    body.appendChild(newDiv);

    profitDiv = guardElement(document.getElementById("profit"), 'profit');
    profitPercentDivAdvisor = guardElement(document.getElementById("profit-percent"), 'profit-percent');
    historyDiv = document.getElementById("history-box");
    sqzDiv = guardElement(document.getElementById("sqz-indicator"), 'sqz-indicator');
    trendDiv = guardElement(document.getElementById("price-trend"), 'price-trend');
    tradingSymbolDiv = guardElement(document.getElementById("trading-symbol"), 'trading-symbol');
    globalTrendDiv = guardElement(document.getElementById("global-trend"), 'global-trend');
    tradeDirectionDiv = guardElement(document.getElementById("trade-dir"), 'trade-dir');
    timeDiv = guardElement(document.getElementById("time"), 'time');
    wonDiv = guardElement(document.getElementById("won-percent"), 'won-percent');
    wagerDiv = guardElement(document.getElementById("wager"), 'wager');
    maxStepDiv = guardElement(document.getElementById("max-step"), 'max-step');
    // cyclesDiv.innerHTML = cyclesToPlay;
    totalProfitDiv = guardElement(document.getElementById("total-profit"), 'total-profit');
    cyclesHistoryDiv = guardElement(document.getElementById("cycles-history"), 'cycles-history');
    signalProgressDiv = guardElement(document.getElementById("signal-progress"), 'signal-progress');
    updateSignalsProgress();
...
const head = guardElement(document.head, 'document.head');
if (head) {
...
rootWindow.__QA__ = {
    ...
    tscheck: true,
    iife: true,
    varsReplaced: QA_COUNTS.varsReplaced,
    loopsFixed: QA_COUNTS.loopsFixed,
    undeclaredFixed: QA_COUNTS.undeclaredFixed,
    protoRemoved: QA_COUNTS.protoRemoved,
    domGuards: QA_COUNTS.domGuards,
    selfTest: selfTestStatus,
    errors: rootWindow.__BOOT_STATUS__.errors,
    warnings: rootWindow.__BOOT_STATUS__.warnings
};
console.groupCollapsed(`%c[QA ${APP_VERSION}] Build checks`, 'color:#1e6fff;font-weight:600;');
console.table(rootWindow.__QA__);
console.groupEnd();

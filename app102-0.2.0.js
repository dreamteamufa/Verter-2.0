...
const ENABLE_DECISION_BY_HTF = false;
...
function linreg(values) {
  ...
  const x = Array.from({ length: period }, (_, idx) => idx + 1);
  const y = values;
  const n = period;
  ...
  const denom = (n * xSqSum) - (xSum * xSum);
  if (denom === 0) {
    return Number.NaN;
  }
...
function trueRange(currentBar, prevClose) {
  const high = Math.max(currentBar.H, prevClose);
  const low = Math.min(currentBar.L, prevClose);
  return high - low;
}

function sqzOne(bars, momentumLength) {
  ...
  const length = 20;
  const subset = bars.slice(-length);
  const closes = subset.map(bar => bar.C);
  const highs = subset.map(bar => bar.H);
  const lows = subset.map(bar => bar.L);
  const basis = sma(closes);
  const dev = stdev(closes) * 2;
  const upperBB = basis + dev;
  const lowerBB = basis - dev;
  const trs = [];
  let prevClose = bars.length > length ? bars[bars.length - length - 1].C : subset[0].C;
  for (let i = 0; i < subset.length; i += 1) {
    const bar = subset[i];
    trs.push(trueRange(bar, prevClose));
    prevClose = bar.C;
  }
  const atr = sma(trs);
  const upperKC = basis + (atr * 1.5);
  const lowerKC = basis - (atr * 1.5);
  const highestHigh = Math.max(...highs);
  const lowestLow = Math.min(...lows);
  const avgHL = (highestHigh + lowestLow) / 2;
  const smaClose = sma(closes);
  const momentumSource = subset.map(price => price - ((avgHL + smaClose) / 2));
  const momentumSlice = momentumSource.slice(-momentumLength);
  return linreg(momentumSlice);
}

function updateWarmup(frames1m) {
  const count = frames1m.length;
  HTF.warmup.EMA10 = Math.min(HTF.need.EMA10, count);
  HTF.warmup.EMA20 = Math.min(HTF.need.EMA20, count);
  HTF.warmup.RSI7 = Math.min(HTF.need.RSI7, Math.max(0, count - 1));
  HTF.warmup.SM10 = Math.min(HTF.need.SM10, count);
  HTF.ready = HTF.warmup.EMA10 >= HTF.need.EMA10
    && HTF.warmup.EMA20 >= HTF.need.EMA20
    && HTF.warmup.RSI7 >= HTF.need.RSI7
    && HTF.warmup.SM10 >= HTF.need.SM10;
}

function updateSignalProgressLine() {
  if (!signalProgressDiv) {
    return;
  }
  const parts = [
    `Signals: EMA10 ${HTF.warmup.EMA10}/${HTF.need.EMA10}`,
    `EMA20 ${HTF.warmup.EMA20}/${HTF.need.EMA20}`,
    `RSI7 ${HTF.warmup.RSI7}/${HTF.need.RSI7}`,
    `SM10 ${HTF.warmup.SM10}/${HTF.need.SM10}`
  ];
  let text = `${parts[0]} • ${parts[1]} • ${parts[2]} • ${parts[3]}`;
  if (HTF.ready) {
    text += ' • READY';
  }
  signalProgressDiv.textContent = text;
}

function logHTFSignals(values) {
  if (!values) {
    return;
  }
  const { ema10Value, ema20Value, rsi7Value, sqz10Value, ts } = values;
  if (HTF.lastLoggedTs === ts) {
    return;
  }
  HTF.lastLoggedTs = ts;
  console.log(
    '%c[HTF '+APP_VERSION+'] EMA10=%s EMA20=%s RSI7=%s SM10=%s | EMA10 %d/10 EMA20 %d/20 RSI7 %d/7 SM10 %d/20%s',
    blue,
    Number.isFinite(ema10Value) ? ema10Value.toFixed(5) : 'n/a',
    Number.isFinite(ema20Value) ? ema20Value.toFixed(5) : 'n/a',
    Number.isFinite(rsi7Value) ? rsi7Value.toFixed(2) : 'n/a',
    Number.isFinite(sqz10Value) ? sqz10Value.toFixed(5) : 'n/a',
    HTF.warmup.EMA10,
    HTF.warmup.EMA20,
    HTF.warmup.RSI7,
    HTF.warmup.SM10,
...
function updateHTFIndicators(closedBar, frames1m) {
  if (!closedBar || !Array.isArray(frames1m)) {
    return;
  }
  const closes = frames1m.map(bar => bar.C);
  const ema10Value = emaOne(closes, HTF.need.EMA10);
  const ema20Value = emaOne(closes, HTF.need.EMA20);
  const rsi7Value = rsiOne(closes, 7);
  const sqz10Value = sqzOne(frames1m, 10);
  if (frames1m.length >= HTF.need.EMA10) {
    HTF.ema10.push(ema10Value);
  }
  if (frames1m.length >= HTF.need.EMA20) {
    HTF.ema20.push(ema20Value);
  }
  if (frames1m.length >= HTF.need.RSI7 + 1) {
    HTF.rsi7.push(rsi7Value);
  }
  if (frames1m.length >= HTF.need.SM10) {
    HTF.sqz10.push(sqz10Value);
  }
  updateWarmup(frames1m);
  updateSignalProgressLine();
  logHTFSignals({ ema10Value, ema20Value, rsi7Value, sqz10Value, ts: closedBar.t });
}
...
let currentProfit = 0;
let totalWager = 0;
let profitDiv, profitPercentDivAdvisor, historyDiv, sqzDiv, trendDiv,
    tradingSymbolDiv, globalTrendDiv, tradeDirectionDiv, timeDiv,
    wonDiv, wagerDiv, maxStepDiv, totalProfitDiv,
    cyclesHistoryDiv, signalProgressDiv;
let cyclesDiv;
let historyBetDiv;
let globalPrice;
    ...
    for(const [lbl,per] of Object.entries(TF)){
      ...
      if(slot>b.t){
        ...
        if(lbl==='1m'){
          ...
          updateHTFIndicators(closedBar, frames[lbl]);
        }
...
function addUI() {
    ...
    newDiv.innerHTML += '<div id="signal-progress">Signals: EMA10 0/10 • EMA20 0/20 • RSI7 0/7 • SM10 0/20</div><br>';
    ...
    signalProgressDiv = document.getElementById("signal-progress");
    updateSignalProgressLine();
...
(function selfTest(){
  ...
  for (let i = 0; i < 25; i += 1) {
    const base = 1 + (i * 0.01);
    bars.push({ O: base, H: base + 0.02, L: base - 0.02, C: base, t: i * 60000 });
  }
  const s10 = sqzOne(bars,10);
  if(!Number.isFinite(e10)||!Number.isFinite(r7)||!Number.isFinite(s10)){
...
console.log('Iteration 1: bannedPatterns=0; missingDecls=0; uiContract=OK; selfTest=PASS');
})();
